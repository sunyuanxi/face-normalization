  ...
  def get_train(self):
    ...
    with tf.name_scope('data_feed'):
      ...
      for i in profile_list:
        j = 'c_' + i.split('_')[-5] + '_01_01_051_10.png'
        target_list += [j]
      target_list = [os.path.join(cfg.front_path, img) for img in target_list]
      front_list = [os.path.join(cfg.front_path,img) for img in self.front]

      #tag_list = []
      #for i in profile_list:
      #  tag_list += [i.split('_')[3]]
      ...
      print(len(profile_list), len(target_list))
      print(profile_list[0])
      print(target_list[0])
 
      profile_queue = tf.FIFOQueue(cfg.dataset_size, [tf.string, tf.string], shapes=[[], []])
      enqueue_op = profile_queue.enqueue_many([profile_list, target_list])
      profile_file, target_file = profile_queue.dequeue()

      front_queue = tf.FIFOQueue(cfg.dataset_size, tf.string)
      front_file = front_queue.dequeue()

      profile_value = tf.read_file(profile_file)
      target_value = tf.read_file(target_file)
      front_value = tf.read_file(front_file)

      #_, profile_value = tf.WholeFileReader().read(prrofile_files)
      #tag = tf.reshape(tf.decode_raw(profile_value, tf.uint8)[-20:-17], [1])
      #tag = tf.decode_raw(profile_value, tf.uint8)[-20:-17]
      #target_value = fea_dict[tag].reshape(2048)
      profile_value = tf.image.decode_jpeg(profile_value, channels=cfg.channel)
      profile_value = tf.cast(profile_value, tf.float32)
      target_value = tf.image.decode_jpeg(target_value, channels=cfg.channel)
      target_value = tf.cast(target_value, tf.float32)
      ...
      target_value = tf.image.resize_images(target_value, [cfg.height, cfg.width])
      ...
      profile, target, front = tf.train.shuffle_batch([profile_value, target_value, front_value],
                          batch_size=self.batch_size,
                          num_threads=8,
                          capacity=32 * self.batch_size,
                          min_after_dequeue=self.batch_size * 16,
                          allow_smaller_final_batch=False)
      return profile, target, front
