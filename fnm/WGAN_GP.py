...
class WGAN_GP(object):
  ...
  def loss(self, profile, front):
    ...
    with tf.name_scope('loss') as scope:
      ...
      # 0. Supervised Loss: Cosine-Norm
      #tag = ??
      #self.target_distance = (1 - tf.reduce_sum(tf.multiply(pool5_gen_p_norm, pool5_t_norm), [1]))
      #self.supervised_loss = tf.reduce_mean(self.target_distance)
      #tf.add_to_collection('losses', self.supervised_loss)
...
#      with tf.name_scope('Symmetric_Loss'):
        ...
        #eyes = tf.slice(self.gen_p, [0,56,44,0], [bs,46,136,cfg.channel])  # [56:102,44:180,:]
        #nose = tf.slice(self.gen_p, [0,70,88,0], [bs,74,48,cfg.channel])  # [70:144,88:136,:]
        #mouth = tf.slice(self.gen_p, [0,144,80,0], [bs,36,64,cfg.channel])  # [144:180,80:144,:]
        #face = tf.slice(self.gen_p, [0,40,34,0], [bs,150,156,cfg.channel])  # [40:190,34:190,:]
        #local_img = [eyes, nose, mouth, face]
        #local_weight = [1, 1, 1, 1]
#        self.sym_loss = 0
#        for local_im, local_w in zip(local_img, local_weight):
#          mirror_p = tf.reverse(local, axis=[2])
#          self.sym_loss = local_w * tf.reduce_mean(tf.reduce_sum(tf.abs(mirror_p/225. - local_im/255.), [1,2,3]))
