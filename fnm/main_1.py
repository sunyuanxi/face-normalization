...
from WGAN_GP_1 import WGAN_GP
from utils_1 import loadData
...
def main(_):
  ...
  profile, target, front = data_feed.get_train()
  net.build_up(profile, target, front)
  ...
  with tf.Session(config=config, graph=net.graph) as sess:
    ...
    for epoch in range(cfg.epoch):
      for step in range(num_batch):        
        ...
        #                                   {net.is_train:True})
        _,fl,gl,dl,tl,gen,g1,g2,g4,summary = sess.run([net.train_gen, net.feature_loss,net.g_loss,
                            net.d_loss,net.supervised_loss,net.gen_p,net.grad1,net.grad2,net.grad4,
                            net.train_summary],{net.is_train:True})
        
        #print('%d-%d, Fea Loss:%.2f, D Loss:%4.1f, G Loss:%4.1f,' % (epoch, step, fl, dl, gl))
        print('%d-%d, Fea Loss:%.2f, D Loss:%4.1f, G Loss:%4.1f, Target Loss:%4.1f, g1/2/4:%.5f/%.5f/%.5f ' %  #
           (epoch, step, fl, dl, gl, tl, g1*cfg.lambda_fea,g2,g4))                 
